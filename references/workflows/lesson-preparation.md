...
使用 `assets/templates/02-teaching-design.md`，并完整执行 `teacher-ready-content.md`。教学过程必须成为正文主体；每个核心活动写清任务与材料、教师组织与关键话语、学生行动、评价与时间；预设回答与引导只写关键处（每个核心活动一至两处，选学生最可能卡住的地方），不逐项穷尽；转场并入组织话语一句带过；超时删减在整课预案统一给出。导入和整课情境必须来自前述研究与比较，但备选立意的比较属研究记录，不写入成稿正文。板书必须给出空间预览和随课堂生成的落笔顺序。常态课教学设计以 1500 至 2500 字为建议区间，优先删说明性文字与重复交代，保留教师开场、情境材料、检测题与作业等成品件；既有检查工具的篇幅提示是非阻断提醒，最终以课堂可用性判断。
