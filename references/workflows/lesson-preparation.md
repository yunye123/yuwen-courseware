...
> 状态：active
> 适用范围：真实课例生产主线
> 核心原则：老师负责目标、取舍与批准；先研究和教学设计，后完成逐字稿与白底课件。学案只在课堂需要时增加。
...
2. **方法验证线**：维护端的 gold/holdout 用于验证方法是否能跨课型复用，以及约束对外的能力声明，不是每次备课的启动门槛，也不进入教师发布包。
3. **真实教师校准线**：按项目规则周期触发，用于校准 AI 评审尺度，不是每包固定动作。
...
| 7 | `teaching-script` | 逐字稿已确认，课时、追问、纠偏和删减方案可执行 |
| 8 | `text-contract` | 教学设计、逐字稿、可选学案与课件所用问题、答案口径已核对一致 |
| 9 | `ppt-text` | 白底课件的逐页内容已确认并可导出；可按需形成外部设计交接单 |
| 10 | `external-ppt-check` | 可选：外部返稿完成内容一致性核对 |
| 11 | `release` | 中文教师成品文件夹与私有课例归档分别完成检查 |

阶段 6 和阶段 10 可选。教学设计确认后，若课堂不需要纸质学案或老师明确不要，直接进入逐字稿；需要时才进入阶段 6。阶段 9 批准后，未请求外部设计时直接进入阶段 11；请求外部设计并把返稿纳入商业交付时才进入阶段 10。

与老师沟通时默认收敛为三个真正需要做教学取舍的决策点：立意选择、教学设计确认、成品验收。教学设计确认时同时判断是否需要学案。逐字稿完成后另有一次独立但极短的“教学内容核对”：宿主必须先给出教学设计、逐字稿与可选学案的完整教师版文件，只请老师确认有没有漏项或前后说法不一致，不展示后台文件，不再要求选路线。其余中间阶段由宿主用中文汇报完整文件、具体内容例子、检查结论和下一步。状态守卫仍在后台逐阶段留存记录。
...
导入、整课情境和课堂组织框架属于每课研究的重点，不是写教案时临时补一个开场。研究计划必须显式提出：学生已有判断与文本之间有什么冲突；哪些文本反常处可以启动核心问题；用户优质案例和一线课堂实录怎样组织导入与任务；候选情境通过“去皮、入文、统摄、成果”四项检验中的哪几项。联网结果只作为创意与实践线索，关键事实和引文仍回到可靠来源核对。
...
使用 `assets/templates/02-teaching-design.md`，并完整执行 `teacher-ready-content.md`。教学过程必须成为正文主体；每个核心活动写清任务与材料、教师组织与关键话语、学生行动、评价与时间；预设回答与引导只写关键处（每个核心活动一至两处，选学生最可能卡住的地方），不逐项穷尽；转场并入组织话语一句带过；超时删减在整课预案统一给出。导入和整课情境必须来自前述研究与比较，但备选立意的比较属研究记录，不写入成稿正文。板书必须给出空间预览和随课堂生成的落笔顺序。教学设计成稿以 3000 至 5000 字为目标；超出时优先删说明性文字与重复交代，不删教师开场、情境材料、检测题与作业等成品件。

教学目标、任务、学生产出、评价、教师支架、时间和删减方案必须一一对应。内部仍冻结 `OBJ/TASK/ASM/ANS/MAT/SCRIPT` 的最终编号与数量，但映射只能写入模板顶部明确标记的 `YUWEN_INTERNAL_MAPPING` 注释或独立内部结构文件；不得生成“下游稳定编号分配表”，也不得把内部编号列放进教师正文。下游按内部映射使用，不在学案、答案或逐字稿阶段临时重编。情境、创意与 AI 只有在直接改善理解、表达、反馈或评价时才保留。
...
使用者看到完整、已清洗的教师版教学设计文件，以及用教研语言写的设计摘要、主要取舍和未决问题后批准。质量审查摘要、来源代码和编号映射留在内部。教学设计未批准，不得生成学案、逐字稿或白底 PPT。
...
- 课堂可以由课件、口头交流、板书或学生自备纸完成，或老师明确不要学案：跳过学案，直接生成逐字稿。
...
随后无论是否有学案，都使用 `assets/templates/05-teaching-script.md` 生成可打印逐字稿。逐字稿第一页给整课路线与时间锚点；正文按课堂发生顺序写可直接说出口的话语，并紧邻安排等待时间、学生反应分支、追问纠偏、翻页与板书动作、转场和超时删减。生成后模拟“只看稿能否顺讲、沉默时能否接住、超时五分钟能否立刻删减”。没有独立教师答案时，必要的参考答案、可接受的不同答案、反馈语和评分依据必须进入逐字稿旁注或课件备注，不能消失。

产物完成后执行跨成品审查：编号、事实、任务顺序、时间、题目与答案必须一致。有学案时核对教学设计、学案、参考答案和逐字稿；无学案时核对教学设计与逐字稿，并要求所有学生任务进入 PPT。确认后才进入课件制作。
...
阶段 9 的课件内容获批后，才整理教师可编辑文件。公开核心版默认交付教学设计 Markdown、授课逐字稿 Markdown 与白底课件 PPTX；选择学案路线时再增加学生学案 Markdown 与教师参考答案 Markdown。老师如需 DOCX/PDF，应使用学校认可的转换工具自行生成，并在目标设备中实际核验。
...
- `content-pack-approved`：教学设计和逐字稿已批准；若选择学案，学案与参考答案也已批准。
- `ppt-text-approved`：白底课件逐页内容已批准，可进入发布检查；也可按需交外部设计。
- `final-package-ready`：教学设计、逐字稿、白底 PPT，以及所选的学案与参考答案齐全；教师交付文件夹与私有课例归档均已分别完成检查。
