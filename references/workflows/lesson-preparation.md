...
生成教师版、清除 HTML 内部注释后，运行 <PYTHON> scripts/check.py <教学设计.md>。检查器只执行机械可判定的教学设计成稿合同检查，不评价教学立意、文本解读深度、问题质量或课堂创新；检查结果只用于内部修改，不进入教师成稿。教学设计是下游设计源：不得为了减字删去核心问题、关键预设或课堂推进逻辑。教学设计完成后默认进入“教学设计确认”；未经老师确认，不得自动生成授课导航稿、按需完整逐字稿或 PPT。
